...
I2C_ESP32::I2C_ESP32(PhysicalPin scl, PhysicalPin sda) : _scl(scl), _sda(sda) {
...
class ADC_ESP32 : public ADC {
 ...
 public:
  ...
  /*    uint32_t voltage =
          adc1_to_voltage((adc1_channel_t)_channel, &_characteristics);*/
      return voltage;
    } else if (_unit == ADC2) {
      int voltage;
      esp_err_t r =
...
class SPI_ESP32 : public Spi {
 ...
 public:
  ...
  int exchange(std::string &in, std::string &out) {
    ...
    if (out.length() == 0) return EINVAL;  // no need to send anything
...
class UART_ESP32 : public UART {
 ...
 public:
  ...
  int init() {
    ...
    std::string taskName = "uart_event_task_";
    taskName += std::to_string(_driver);
    xTaskCreate(uart_event_task, taskName.c_str(), 5120, this,
                /*tskIDLE_PRIORITY + 5*/ 20,
                &_taskHandle);  // increased to 5120
    return 0;
  }
  ...
  int write(uint8_t b) {
    if (uart_write_bytes(_uartNum, (const char *)&b, 1) == 1) return 0;
    return 0;
  }
