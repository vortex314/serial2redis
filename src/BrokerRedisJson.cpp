...
int BrokerRedis::connect(std::string node) {
  ...
  redisAsyncSetPushCallback(_asyncContext, onPush);
  ...
  _thread.addReadInvoker(_asyncContext->c.fd,
                         [&](int) { redisAsyncHandleRead(_asyncContext); });
  _thread.addWriteInvoker(_asyncContext->c.fd,
                          [&](int) { redisAsyncHandleWrite(_asyncContext); });
  connected = true;
  return 0;
}
...
int BrokerRedis::unSubscribe(std::string pattern) {
  ...
  if (it != _subscriptions.end()) {
    ...
    int rc = redisAsyncCommand(_asyncContext, onReply, this, "PUNSUBSCRIBE %s",
                               pattern.c_str());
    if (rc != REDIS_OK) {
...
int BrokerRedis::publish(std::string topic, const std::string &bs) {
  ...
  int rc = redisAsyncCommand(_asyncContext, onReply, this, "PUBLISH %s %b",
                             topic.c_str(), bs.data(), bs.size());
  if (rc == REDIS_OK) {
...
int BrokerRedis::command(AsyncCallback func, const char *format, ...) {
  INFO("%s", format);
  if (!connected()) return ENOTCONN;
  va_list ap;
  va_start(ap, format);
  int rc = redisvAsyncCommand(_asyncContext, func, this, format, ap);
  va_end(ap);
...
redisReply *BrokerRedis::xread(std::string key) {
  if (!connected()) return 0;
  return (redisReply *)redisAsyncCommand(
      _asyncContext, onReply, this, "XREAD BLOCK 0 STREAMS %s $", key.c_str());
}